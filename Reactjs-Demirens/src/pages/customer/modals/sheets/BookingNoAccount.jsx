...
function BookingNoaccount({ rooms, selectedRoom, guestNumber: initialGuestNumber, adultNumber, childrenNumber, extraGuestPrice, bedPrice }) {
  ...
  const preserveScroll = (fn) => {
    try {
      ...
      const vpBefore = document.querySelector(vpSelector);
      const prevTop = vpBefore ? vpBefore.scrollTop : null;
      fn();
      requestAnimationFrame(() => {
        const vpAfter = document.querySelector(vpSelector);
        if (vpAfter != null && prevTop != null) {
          vpAfter.scrollTop = prevTop;
        }
      });
    } catch {
      fn();
